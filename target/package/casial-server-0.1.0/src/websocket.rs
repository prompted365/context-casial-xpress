...
impl WebSocketHandler {
        ...
        while let Some(msg) = ws_receiver.next().await {
            match msg {
                ...
                Ok(Message::Ping(ping)) => {
                    ...
                    let pong_msg = Message::Pong(ping);
    ...
    async fn handle_tools_call(
        ...
        let coordination_result = {
            let mut engine = self.state.casial_engine.write();
    ...
    async fn handle_casial_debug(
        ...
        let engine_stats = {
            let engine = self.casial_engine.read();
            ...
            serde_json::json!({
                ...
                "active_paradoxes": paradox_registry.iter().filter(|p| p.resolved_at.is_none()).count(),
                "resolved_paradoxes": paradox_registry.iter().filter(|p| p.resolved_at.is_some()).count(),
