...
async fn load_mission(state: &AppState, mission_path: PathBuf) -> Result<()> {
    ...
    {
        let mut engine = state.casial_engine.write();
...
async fn build_router(state: AppState) -> Result<Router> {
    let router = Router::new()
        ...
        // Middleware
        .layer(
            ServiceBuilder::new().layer(create_cors_layer()).layer(
...
async fn debug_missions(State(state): State<AppState>) -> impl IntoResponse {
    let missions = state.mission_manager.read().get_all_missions();
...
async fn debug_paradoxes(State(state): State<AppState>) -> impl IntoResponse {
    ...
    axum::Json(serde_json::json!({
        "paradoxes": paradoxes.iter().map(|p| serde_json::json!({
            ...
            "resolved": p.resolved_at.is_some(),
            "conflicting_elements": p.conflicting_elements.len()
        })).collect::<Vec<_>>()
    }))
}
...
async fn debug_perceptions(
    ...
    let engine = state.casial_engine.read();

    // Get current mission perceptions
    let perceptions = if let Some(mission) = engine.get_current_mission() {
        mission
            .perceptions
            .iter()
            .map(|p| {
                serde_json::json!({
                    ...
                    "perception_type": format!("{:?}", p.perception_type)
                })
            })
            .collect::<Vec<_>>()
    } else {
    ...
    let debug_info = serde_json::json!({
        "perceptions": {
            "total_count": perceptions.len(),
            "locked_count": perceptions.iter().filter(|p| p["locked"].as_bool().unwrap_or(false)).count(),
            "avg_confidence": {
                let confidences: Vec<f64> = perceptions.iter()
                    .filter_map(|p| p["confidence"].as_f64())
                    .collect();
                if confidences.is_empty() { 0.0 } else { confidences.iter().sum::<f64>() / confidences.len() as f64 }
            },
            "perceptions_list": perceptions
        },
...
async fn debug_sprawl(
    ...
    // Analyze templates from the casial engine
    {
        ...
        if let Some(mission) = engine.get_current_mission() {
